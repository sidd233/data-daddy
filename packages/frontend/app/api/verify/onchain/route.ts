...
const DEMO_WALLET = (process.env.DEMO_WALLET_ADDRESS ?? "").toLowerCase();
const DEMO_CACHE_PATH = path.join(process.cwd(), "demo-wallet-cache.json");
...
export async function GET(request: NextRequest) {
  ...
  // ── Demo wallet: return cached attributes, skip live Alchemy calls ──────────
  if (DEMO_WALLET && address === DEMO_WALLET) {
    const attributes: Attribute[] = JSON.parse(
      fs.readFileSync(DEMO_CACHE_PATH, "utf-8")
    );

    // Still upsert to DB and mint certs so the demo has real on-chain state
    const upsertResults: { id: number; certificate_token_id: number | null }[] = [];
    for (const attr of attributes) {
      const { rows } = await pool.query<{ id: number; certificate_token_id: number | null }>(
    ...
    for (let i = 0; i < attributes.length; i++) {
      ...
      if (attr.verified && !row.certificate_token_id) {
        try {
          await mintCertificate(address, attr.attribute, Math.round(attr.confidence * 100), row.id);
        } catch (mintErr) {
          console.error(`Demo mint failed for ${attr.attribute}:`, mintErr);
        }
      }
    }
